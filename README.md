...
| `kubevirt_image_pull_grace` | Seconds a containerDisk pull may sit in `ImagePullBackOff`/`ErrImagePull` before provisioning gives up. kubelet retries pulls, so a cold pull-through cache, a slow upstream registry or a dropped connection normally clears on a later attempt. Bounded by the effective `timeout` (which this does not raise), and skipped when the failure is definitive — unknown manifest, `unauthorized`, `denied` — so a typo'd tag still fails immediately. Set it to `0` to switch the window off and fail on the first backoff, as provisioning did before this option existed. | No | `600` | HOSTS (per-host), CONFIG |
